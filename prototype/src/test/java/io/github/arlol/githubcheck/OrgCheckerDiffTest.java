...
class OrgCheckerDiffTest {
    ...
    private static RepositoryState goodArchivedState(String name) {
        return new RepositoryState(
                ...
                false, // secretScanningPushProtection — not checked for archived
    ...
    void noDrift_forCorrectPublicRepo() {
        List<String> diffs = checker.computeDiffs(goodPublicState("repo"), defaultArgs());
    ...
    void noDrift_forCorrectArchivedRepo() {
        ...
        List<String> diffs = checker.computeDiffs(goodArchivedState("repo"), args);
        assertThat(diffs).isEmpty();
    }
    ...
    void drift_allowMergeCommit_isTrue() {
        RepositoryState state = new RepositoryState(
                ...
                List.of("check-actions.required-status-check",
                        "codeql-analysis.required-status-check", "CodeQL", "zizmor"),
                List.of(), Map.of(), "read", true);
    ...
    void drift_secretScanningPushProtection_isFalse() {
        RepositoryState state = new RepositoryState(
                ...
                List.of(), Map.of(), "read", true);
        assertThat(checker.computeDiffs(state, defaultArgs()))
                .contains("secret_scanning_push_protection: want=true got=false");
    }

    @Test
    void archived_skipsSecurityAndBranchProtectionChecks() {
        // An archived repo that has wrong security settings — should not flag them
        RepositoryState state = new RepositoryState(
                "repo", true, "public",
                false, false, true, true,
                false, false, false, // security all false — should be ignored for archived
                false, false, false, // branch protection all false — should be ignored for archived
                List.of(),
                List.of(), Map.of(), "read", false);
        RepositoryArgs args = RepositoryArgs.builder().archived().build();
    ...
    void privateRepo_skipsBranchProtectionCheck() {
        // A private repo with correct other settings but no branch protection — ok
        RepositoryState state = new RepositoryState(
                "repo", false, "private",
                false, false, true, true, true, true, true,
                false, false, false, // no branch protection — ok for private
                List.of(),
                List.of(), Map.of(), "read", true);
    ...
    void drift_enforceAdmins_isFalse() {
        RepositoryState state = new RepositoryState(
                ...
                List.of(), Map.of(), "read", true);
        assertThat(checker.computeDiffs(state, defaultArgs()))
                .contains("branch_protection.enforce_admins: want=true got=false");
    }
    ...
    void drift_requiredLinearHistory_isFalse() {
        RepositoryState state = new RepositoryState(
                ...
                List.of(), Map.of(), "read", true);
        assertThat(checker.computeDiffs(state, defaultArgs()))
                .contains("branch_protection.required_linear_history: want=true got=false");
    }
    ...
    void drift_missingBaseStatusCheck() {
        RepositoryState state = new RepositoryState(
                ...
                List.of("check-actions.required-status-check",
                        "codeql-analysis.required-status-check",
                        "CodeQL"), // missing "zizmor"
                List.of(), Map.of(), "read", true);
        assertThat(checker.computeDiffs(state, defaultArgs()))
                .anyMatch(d -> d.contains("branch_protection.required_status_checks")
                        && d.contains("missing")
                        && d.contains("zizmor"));
    }
    ...
    void drift_extraUnexpectedStatusCheck() {
        RepositoryState state = new RepositoryState(
                ...
                List.of("check-actions.required-status-check",
                        "codeql-analysis.required-status-check", "CodeQL", "zizmor",
                        "unexpected-check"), // extra check
                List.of(), Map.of(), "read", true);
        assertThat(checker.computeDiffs(state, defaultArgs()))
                .anyMatch(d -> d.contains("branch_protection.required_status_checks")
                        && d.contains("extra")
                        && d.contains("unexpected-check"));
    }
    ...
    void drift_missingActionSecret() {
        ...
        RepositoryState state = new RepositoryState(
                ...
                Map.of(), "read", true);
        assertThat(checker.computeDiffs(state, args))
                .anyMatch(d -> d.contains("action_secrets")
                        && d.contains("missing")
                        && d.contains("PAT"));
    }
    ...
    void drift_missingEnvironment() {
        ...
        RepositoryState state = new RepositoryState(
                ...
                "read", true);
        assertThat(checker.computeDiffs(state, args))
                .anyMatch(d -> d.contains("environments")
                        && d.contains("missing")
    ...
    void drift_missingEnvironmentSecret() {
        ...
        RepositoryState state = new RepositoryState(
                ...
                List.of("check-actions.required-status-check",
                        "codeql-analysis.required-status-check", "CodeQL", "zizmor"),
                List.of(),
                Map.of("production", List.of()), // production exists but no secrets
                "read", true);
        assertThat(checker.computeDiffs(state, args))
                .anyMatch(d -> d.contains("environment.production.secrets")
                        && d.contains("missing")
    ...
    void drift_canApprovePullRequestReviews_isFalse() {
        ...
        assertThat(checker.computeDiffs(state, defaultArgs()))
                .contains("workflow_permissions.can_approve_prs: want=true got=false");
    }
